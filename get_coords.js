...
const INPUT = 'verified_restaurants.json';
const CACHE_FILE = 'coords_cache.json';
const DELAY_MS = 1200;
const SAVE_EVERY = 5 * 60 * 1000; // 5 dakika
...
async function process() {
    ...
    let withCoords = restaurants.filter(r => r.lat && r.lat != 0 && isTurkey(r.lat, r.lng));
    let found = withCoords.length;
    let processed = found;
    let lastSave = Date.now();
    
    console.log(`Başlangıç: ${found}/${total} Türkiye koordinat`);
    ...
    for (let i = 0; i < total; i++) {
        ...
        if (Date.now() - lastSave > SAVE_EVERY) {
            ...
            const backup = `verified_${ts}.json`;
            fs.writeFileSync(backup, JSON.stringify({ restaurants }, null, 2));
